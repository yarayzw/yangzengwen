        ...
        <div class="hd-main">
            ...
            <div class="hd-nav clearfix">
                ...
                <dl class="hd-account clearfix">
                    ...
                    <dd>
                        <a class="hd-name" href="##"><?php echo (session('name')); ?></a>
       ...
       <div style="width:750px;height:500px;border:1px #ccc solid;overflow:auto;background:#ffffff;">
           ...
           <ul>
               <?php if(is_array($photopath)): foreach($photopath as $key=>$vo): ?><li style="float:left;width:150px;margin-left:15px;margin-top:15px;"><img width="150" height="150" onclick="scanphoto(this)" alt="<?php echo ($vo["path"]); ?>" src="/thinkphp/Public/Uploads/img/<?php echo ($vo["path"]); ?>">
                       <a  onclick="setup(this)" pid="<?php echo ($vo["pid"]); ?>"  alt="<?php echo ($vo["id"]); ?>" minpath="<?php echo ($vo["path"]); ?>" href="javascript:viod(0)"><img src="/thinkphp/Public/Uploads/004.jpg"></a>
                       <a  onclick="delet(this)" alt="<?php echo ($vo["id"]); ?>" path="<?php echo ($vo["path"]); ?>" pid="<?php echo ($vo["pid"]); ?>"  href="javascript:viod(0)"><img src="/thinkphp/Public/Uploads/dele.png"></a>
                   </li><?php endforeach; endif; ?>
           </ul>
       </div>

        <form id="f0" action="/thinkphp/index.php/Home/Home/delpic" method="post" style="display:none">
            <input id="delehid" type="hidden" name="delete" value="">
            <input id="path" type="hidden" name="path" value="">
            <input id="pid" type="hidden" name="pid" value="">
            <input type="hidden" name="setup3" value="<?php echo ($content); ?>">
            <input type="submit">
        </form>
        <form id="f1" action="/thinkphp/index.php/Home/Home/setup" method="post" style="display:none">
            <input id="setup0" type="hidden" name="setup0" value="">
            <input id="setup1" type="hidden" name="setup1" value="">
            <input id="setup2" type="hidden" name="setminpath" value="">
            <input type="submit">
        </form>
...
<script>
    ...
    function delet(ob){
        var id=$(ob).attr('alt');
        var path=$(ob).attr('path');
        var pid=$(ob).attr('pid');
        $('#delehid').attr('value',id);
    ...
    function setup(ob){
        var id=$(ob).attr('alt');
        var pid=$(ob).attr('pid');
        var minpath=$(ob).attr('minpath');
        $('#setup0').attr('value',id);
        $('#setup1').attr('value',pid);
        $('#setup2').attr('value',minpath);
        layer.msg('设置为封面？', {
            time: 0 //不自动关闭
            ,btn: ['确定', '取消']
            ,yes: function(index){
