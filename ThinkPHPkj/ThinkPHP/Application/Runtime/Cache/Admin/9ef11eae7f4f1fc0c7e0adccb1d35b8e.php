...
<head>
    ...
    <div id="divid" style="display: none;" align="center">
        <table  align="center" style="text-align: center">
            <form method="post" action="/thinkphp/index.php/Admin/User/addUser" onsubmit="return checkSubmit()" id="formid">
                <tr>
                ...
                <tr>
                    ...
                    <td>身份：<select name="roleid">
                        <?php if(is_array($result)): foreach($result as $key=>$vv): ?><option value="<?php echo ($vv["roleid"]); ?>"><?php echo ($vv["rolename"]); ?></option><?php endforeach; endif; ?>
                        </select>
                    </td>
                </tr>
                <tr>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" id="search">
                <tr>
                    <td width="90%" align="left" valign="middle">
                        <form method="post" action="/thinkphp/index.php/Admin/User/pageUser">
                            <span>管理员：</span>
    ...
    <tr>
        <td align="left" valign="top">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
                ...
                <?php if(is_array($res)): $i = 0; $__LIST__ = $res;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
                        ...
                        <td align='center' valign='middle' class='borderright'><?php echo ($row['sex']==0?$sex[0]:$sex[1]); ?></td>
                        <td align='center' valign='middle' class='borderright'><?php echo ($row['tel']); ?></td>
                        <td align='center' valign='middle' class='borderright'>
                            <?php if(is_array($result)): foreach($result as $key=>$vov): if($vov['roleid'] == $row['roleid']): echo ($vov["rolename"]); endif; endforeach; endif; ?>
                        </td>
                        <td align='center' valign='middle' class='borderright'>
                            <?php if($row['state'] == 1): ?>无限制<?php endif; ?>
                            <?php if($row['state'] == 2): ?>禁止<?php endif; ?>
                        </td>
                        <td align='center' valign='middle' class='borderright'><?php echo ($row['addtime']); ?></td>
                        <td align="center" valign="middle" class="borderbottom">
                            <span class="gray"></span><a href="/thinkphp/index.php/Admin/User/delUser?id=<?php echo ($row['id']); ?>">删除</a>
                            <span class="gray"></span><a href="/thinkphp/index.php/Admin/User/mdfUser?id=<?php echo ($row['id']); ?>">修改</a>
                        </td>
...
<script>
    ...
    $('#username').blur(function () {
        ...
        if(!text.test($('#username').val())){
            ...
            $.post("/thinkphp/index.php/Admin/User/inpectUser",{buser:buser},function(data){
                if(data==1){
                    $('#user').html('该用户已存在');
                }
            },'json');
